...
struct config *prepare_default_config()
{
	...
	config->sleep = 25000;
	config->load  = 25000;
	config->sleep_step = 25000;
	config->load_step  = 25000;
	config->cycles = 20;
	config->rounds = 20;
	config->cpu = 1;
	...
	/* Reset string */
	  memset(config->governor, '\0', sizeof(config->governor));
	//__android_log_print(ANDROID_LOG_INFO,"parse.c",config->governor);
	strncpy(config->governor, "ondemand", 9);
	//__android_log_print(ANDROID_LOG_INFO,"parse.c",config->governor);
	...
	{
		__android_log_print(ANDROID_LOG_INFO,"parse.c","DEFAULT_CONFIG_FILE");
		return NULL;
	}
#endif
	return config;
