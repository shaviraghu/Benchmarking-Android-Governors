...
{
	char path[SYSFS_PATH_MAX];
	char suCmd[SYSFS_PATH_MAX+15];
	int fd;
	ssize_t numwrite;

	snprintf(path, sizeof(path), PATH_TO_CPU "cpu%u/cpufreq/%s",
			 cpu, fname);
	snprintf(suCmd,sizeof(suCmd),"su -c chmod 646 /sys/devices/system/cpu/cpu%u/cpufreq/scaling_governor",cpu);
	__android_log_print(ANDROID_LOG_INFO,"sysfs.c","path: %s, sizeOfPath: %u Path to cpu: %s cpu%u/cpufreq/%s",path,sizeof(path),PATH_TO_CPU,
			 cpu, fname);

	char command[sizeof(path)+sizeof(value)+20];

	snprintf(command, sizeof(command),"echo %s > %s", value,path);

	if(access(path, F_OK==-1))
	{
		__android_log_print(ANDROID_LOG_ERROR,"sysfs.c","file does not exist %s",path);
		return 1;
	}
	...
	if(access(path, W_OK)==-1)
	{
		...
		sleep(1);
		if(access(path, W_OK)==-1)
		__android_log_print(ANDROID_LOG_ERROR,"sysfs.c","no write permissions. Try %s %s",command,suCmd);
	...
	if (fd == -1)
	{
		__android_log_print(ANDROID_LOG_ERROR,"sysfs.c","file opening failed");
		return 0;
	}
	numwrite = write(fd, value, len);
	if (numwrite < 1) {
		__android_log_print(ANDROID_LOG_ERROR,"sysfs.c","file write failed");
		close(fd);
		return 0;
	}
...
int sysfs_modify_freq_policy_governor(unsigned int cpu, char *governor)
{
	...
	{
		__android_log_print(ANDROID_LOG_ERROR,"sysfs.c","governor %s not verified",governor);
		return -EINVAL;
	}
	__android_log_print(ANDROID_LOG_ERROR,"sysfs.c","governor %s verified",governor);
